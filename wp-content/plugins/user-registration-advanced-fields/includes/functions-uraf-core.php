...
function uraf_is_compatible() {
	...
	$ur_plugins_path     = WP_PLUGIN_DIR . URAF_DS . 'user-registration' . URAF_DS . 'user-registration.php';
	...
	if ( ! file_exists( $ur_plugins_path ) && ! file_exists( $ur_pro_plugins_path ) ) {
			return __( 'Please install <code>user-registration-pro</code> plugin to use <code>user-registration-advanced_fields</code> addon.', 'user-registration-advanced-fields' );
	}
	...
	if ( ! is_plugin_active( $ur_plugin_file_path ) && ! is_plugin_active( $ur_pro_plugin_file_path ) ) {
		return __( 'Please activate <code>user-registration-pro</code> plugin to use <code>user-registration-advanced_fields</code> addon.', 'user-registration-advanced-fields' );
	}
	...
	if ( ! is_plugin_active( $ur_pro_plugin_file_path ) ) {
		...
		if ( version_compare( $user_registration_version, '3.0.0', '<' ) ) {
			return __( 'Please update your <code>user registration-pro</code> plugin to at least 3.0.0 version to use <code>user-registration-advanced-fields</code> addon.', 'user-registration-advanced-fields' );
		}
	}
