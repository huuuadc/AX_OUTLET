...
class URCL_Frontend {
	...
	public function assign_user_role_conditionally( $user_data, $form_id, $user_id ) {
		global $table_prefix;
		// check if assign user role conditionally enabled or not.
		$enable_user_role_conditional = ur_get_single_post_meta( $form_id, 'user_registration_form_setting_enable_assign_user_role_conditionally', 'no' );

		if ( ! ur_string_to_bool( $enable_user_role_conditional ) ) {
			return;
		}
	 ...
	 *
	 * @param string|array $left Left value.
	 * @param string       $right Right value.
	 * @param string       $rule_operator Operator.
	 *
	 * @return boolean
	 */
	public static function logic_gates( $left, $right, $rule_operator ) {
		$pass_rule = false;
		switch ( $rule_operator ) {
			...
			case 'less_than':
				...
				break;
			case 'in':
				$pass_rule = is_array( $left ) && in_array( $right, $left, true );
				break;
			case 'not_in':
				$pass_rule = is_array( $left ) && ! in_array( $right, $left, true );
				break;
	...
	public function frontend_scripts( $form_data_array, $form_id ) {
		...
		if( $form_id ){
			$enable_conditional_logic_submit = ur_get_single_post_meta( $form_id, 'user_registration_form_setting_enable_submit_conditional_logic', false );

			if( $enable_conditional_logic_submit ){
				wp_register_script( 'urcl-frontend-conditional-logic-submit-script', URCL()->plugin_url() . '/assets/js/frontend/conditional-logic-submit-frontend' . $suffix . '.js', array( 'jquery' ), URCL_VERSION, true );
				wp_enqueue_script( 'urcl-frontend-conditional-logic-submit-script' );
			}
		}
	}
	...
	public function include_conditions( $filter_data ) {
		...
		if ( ! ur_string_to_bool( $enable_conditional_logic ) ) {
			return $filter_data;
		}
		// get this field id
