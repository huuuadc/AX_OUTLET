...
class WC_Brands_Block_Templates {
	...
	/**
	 * Get the taxonomy-brand template from DB in case a user customized it in FSE
	 *
	 * @return WP_Post|null The taxonomy-brand
	 */
	private function get_brand_template_db() {
		$posts = get_posts(
			array(
				'name'           => 'taxonomy-brand',
	 ...
	 * Without this, the system will always load archive-product.php version instead of taxonomy_brand.html
	...
	public function has_block_template( $has_template, $template_name ) {
		if ( 'archive-product' === $template_name || 'taxonomy-brand' === $template_name ) {
			$has_template = true;
		}
	 ...
	 *
	 * @return WP_Block_Template The taxonomy-brand template.
	 */
	private function get_brands_template( $template_type ) {
		$template_db = $this->get_brand_template_db();
		...
		$template_path = BlockTemplateUtilsDuplicated::should_use_blockified_product_grid_templates()
			? plugin_dir_path( dirname( __FILE__ ) ) . 'templates/blockified/taxonomy-brand.html'
			: plugin_dir_path( dirname( __FILE__ ) ) . 'templates/taxonomy-brand.html';

		$template_file = BlockTemplateUtilsDuplicated::create_new_block_template_object( $template_path, $template_type, 'taxonomy-brand', false );

		return BlockTemplateUtilsDuplicated::gutenberg_build_template_result_from_file( $template_file, $template_type );
	}

	/**
	 * Function to check if a template name is woocommerce/taxonomy-brand
	 *
	 * Notice depending on the version of WooCommerce this could be:
	 *
	 * woocommerce//taxonomy-brand
	 * woocommerce/woocommerce//taxonomy-brand
	 *
	 * @param  String $id The string to check if contains the template name
	 *
	 * @return bool True if the template is woocommerce/taxonomy-brand
	 */
	private function is_taxonomy_brand_template( $id ) {
		return strpos( $id, 'woocommerce//taxonomy-brand' ) !== false;
	}
	 ...
	 * @param string                 $template_type The post_type for the template. Normally wp_template or wp_template_part
	 *
	 * @return WP_Block_Template|null The taxonomy-brand template.
	 */
	public function get_block_file_template( $block_template, $id, $template_type ) {
		if ( $this->is_taxonomy_brand_template( $id ) && is_null( $block_template ) ) {
			$block_template = $this->get_brands_template( $template_type );
		}
	...
	public function get_block_templates( $query_result, $query, $template_type ) {
		...
		if (
			in_array( 'taxonomy-brand', $slugs, true ) ||
			( ! $post_id && ! count( $slugs ) ) ||
			( ! count( $slugs ) && $this->is_taxonomy_brand_template( $post_id ) )
		) {
			$query_result[] = $this->get_brands_template( $template_type );
		}
