...
class WC_Brands {
	...
	public function register_hooks() {
		...
		add_action( 'woocommerce_new_product', array( $this, 'duplicate_add_brand_terms' ) );
	...
	public function update_product_query_tax_query( array $tax_query, WC_Query $wc_query ) {
		if ( isset( $_GET['filter_brand'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$filter_brand = wc_clean( wp_unslash( $_GET['filter_brand'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$brands_filter        = array_filter( array_map( 'absint', explode( ',', $filter_brand ) ) );
	...
	/**
	 * Filter to allow brand in the permalinks for products.
	 *
	 * @access public
	...
	public function post_type_link( $permalink, $post ) {
		...
		if ( empty( $terms ) ) {
			...
			$brand = _x( 'uncategorized', 'slug', 'woocommerce-brands' );
		} else {
			// Replace the placeholder rewrite tag with the first term's slug
			$first_term = array_shift( $terms );
			$brand = $first_term->slug;
		}
	...
	public static function init_taxonomy() {
		...
		register_taxonomy( 'brand',
			array('product'),
			apply_filters( 'register_taxonomy_brand', array(
				'hierarchical'          => true,
	...
	public function brand_description() {
		...
		$term = get_term_by( 'slug', get_query_var( 'term' ), 'brand' );
	...
	public function register_shortcodes() {
		...
		add_shortcode( 'brand', array( $this, 'output_brand' ) );
		add_shortcode( 'brand_thumbnails', array( $this, 'output_brand_thumbnails' ) );
		add_shortcode( 'brand_thumbnails_description', array( $this, 'output_brand_thumbnails_description' ) );
		add_shortcode( 'brand_thumbnails_slider', array( $this, 'output_brand_thumbnails_slider' ) );
		add_shortcode( 'brand_menu', array( $this, 'output_brand_menu' ) );
		add_shortcode( 'brand_list', array( $this, 'output_brand_list' ) );
		add_shortcode( 'brand_products', array( $this, 'output_brand_products' ) );

	}

	/**
	 * output_brand function.
	 *
	 * @access public
	 *
	 ...
	 */
	public function output_brand( $atts ) {
		global $post;
		...
		$brands = wp_get_post_terms( $args['post_id'], 'brand', [ 'fields' => 'ids' ] );
		...
		foreach ( $brands as $brand ) {
			...
			$args['term']      = get_term_by( 'id', $brand, 'brand' );
	 ...
	 * @return string
	 */
	public function output_brand_list( $atts ) {
		...
		$brands = [];
		$terms          = get_terms( 'brand', [ 'hide_empty' => ( $show_empty_brands ? false : true ) ] );
	...
	/**
	 * output_brand_thumbnails function.
	 *
	 * @access public
	 * @param mixed $atts
	 * @return void
	 */
	public function output_brand_thumbnails( $atts ) {
	...
	/**
	 * output_brand_thumbnails_description function.
	 *
	 * @access public
	 * @param mixed $atts
	 * @return void
	 */
	public function output_brand_thumbnails_description( $atts ) {
		...
		wc_get_template( 'widgets/brand-thumbnails-description.php', array(
            ...
            'hide_thumbnail' => $args['hide_thumbnail'],
		), 'woocommerce-brands', untrailingslashit( plugin_dir_path( dirname( __FILE__ ) ) ) . '/templates/' );
	...
	/**
	 * output_brand_thumbnails_slider function.
	 *
	 * @access public
	 * @param mixed $atts
	 * @return void
	 */
	public function output_brand_thumbnails_slider( $atts ) {

        wp_register_style( 'slick_style', '//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.css');
        wp_register_style( 'slick-theme_style', '//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick-theme.min.css');
        wp_enqueue_style( 'slick_style' );
        wp_enqueue_style( 'slick-theme_style' );
        wp_register_script( 'slick_script', '//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js', array(), '1.0.0', true );
		...
		$exclude = array_map( 'intval', explode( ',', $args['exclude'] ) );
		$order   = 'name' === $args['orderby'] ? 'DESC' : 'ASC';
		...
		wc_get_template( 'widgets/brand-thumbnails-slider.php', array(
			'brands'         => $brands,
			'columns'        => $args['columns'],
			'hide_thumbnail' => $args['hide_thumbnail'],
		), 'woocommerce-brands', untrailingslashit( plugin_dir_path( dirname( __FILE__ ) ) ) . '/templates/' );
    ...
    /**
     * output_brand_menu function.
     *
     * @access public
     * @param mixed $atts
     * @return void
     */
    public function output_brand_menu( $atts ) {
        ...
        wc_get_template( 'widgets/brand-menu.php', array(
            'brands'         => $brands,
        ), 'woocommerce-brands', untrailingslashit( plugin_dir_path( dirname( __FILE__ ) ) ) . '/templates/' );
	...
	public function rest_api_maybe_set_brands( $post, $request ) {
		if ( isset( $request['brands'] ) && is_array( $request['brands'] ) ) {
			...
			wp_set_object_terms( $post->ID, $terms, 'brand' );
		}
	}
	...
	public function rest_api_prepare_brands_to_product( $response, $post ) {
		...
		if ( empty( $response->data['brands'] ) ) {
			...
			foreach ( wp_get_post_terms( $post_id, 'brand' ) as $term ) {
				$terms[] = array(
					'id'   => $term->term_id,
					'name' => $term->name,
					'slug' => $term->slug,
	...
	public function rest_api_add_brands_to_product( $product, $request, $creating = true ) {
		...
		if ( ! empty( $brands ) ) {
			...
			wp_set_object_terms( $product_id, $brands, 'brand' );
		}
	}

	/**
	 * Filters products by taxonomy brand.
	 *
	...
	public function woocommerce_brands_update_layered_nav_link( $term_html, $term, $link, $count ) {
		...
		$filter_brand = wc_clean( wp_unslash( $_GET['filter_brand'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$current_attributes   = array_map( 'intval', explode( ',', $filter_brand ) );
		$current_values       = ! empty( $current_attributes ) ? $current_attributes : array();
		$link                 = add_query_arg(
			array(
				'filtering'            => '1',
				'filter_brand' => implode( ',', $current_values ),
			),
	...
	public function duplicate_store_temporary_brands( $duplicate, $original ) {
		$terms = get_the_terms( $original->get_id(), 'brand' );
		if ( ! is_array( $terms ) ) {
			return;
		}
	 ...
	 */
	public function duplicate_add_brand_terms( $product_id ) {
		...
		$term_taxonomy_ids = wp_set_object_terms( $product_id, $term_ids, 'brand' );
