...
class WCBrandsAdminTest extends WP_UnitTestCase {
	 ...
	 * @return void
	 */
	public function test_brand_filter_render_outputs_a_dropdown() {
		...
		wp_set_post_terms( $simple_product->get_id(), [ $term_a_id, $term_b_id, $term_c_id ], 'brand' );

		add_filter(
			'woocommerce_brand_filter_threshold',
			function() {
				return 3;
			}
		);
		...
		ob_start();
		$brands_admin->render_brand_filter();
		$output = ob_get_contents();
		ob_end_clean();

		$this->assertStringContainsString(
			'<select  name=\'brand\' id=\'brand\' class=\'dropdown_brand\'',
			$output
		);
	}
	 ...
	 * @return void
	 */
	public function test_brand_filter_render_outputs_a_select() {
		$simple_product = WC_Helper_Product::create_simple_product();
		...
		$this->assertStringContainsString(
			'<select class="wc-brands-search" name="brand" data-placeholder="Filter by brand" data-allow_clear="true"',
			$output
		);
	}
}
