...
<ul class="brand-thumbnails-description">
	...
	<?php foreach ( $brands as $index => $brand ) :
		...
		<li class="<?php echo esc_attr( $class ); ?>" style="width: <?php echo esc_attr( $width ); ?>%;">
            <?php if($hide_thumbnail!=='true'): ?>
			<a href="<?php echo esc_url( get_term_link( $brand->slug, 'brand' ) ); ?>" title="<?php echo esc_attr( $brand->name ); ?>" class="term-thumbnail">
			...
			<div id="term-<?php echo esc_attr( $brand->term_id ); ?>" class="term-description">
                <h3 class="brand_name"><a href="<?php echo esc_url( get_term_link( $brand->slug, 'brand' ) ); ?>" title="<?php echo esc_attr( $brand->name ); ?>"><?php echo $brand->name; ?></a></h3>
