...
final class WOOF {
    ...
    public function init() {
        ...
        add_action('woocommerce_after_shop_loop', array($this, 'woocommerce_after_shop_loop'), 9999);
    ...
    public function woocommerce_product_query($q) {
        ...
        if(isset($q->query)) {
            if(isset($q->query['product_cat'])) {
                $cat = explode(',', $q->query['product_cat']);
                if(count($cat) > 1) {
                    array_shift($cat);
                    $q->query['product_cat'] = implode(',', $cat);
                }
            }
        }
        if(isset($q->query_vars)) {
            if(isset($q->query_vars['product_cat'])) {
                $cat = explode(',', $q->query_vars['product_cat']);
                if(count($cat) > 1) {
                    array_shift($cat);
                    $q->query_vars['product_cat'] = implode(',', $cat);
                }
            }
        }
        //var_dump($q->query);
        ...
        if ($wp_query->is_main_query()) {
            if ($this->is_isset_in_request_data($this->get_swoof_search_slug())) {
                ...
                if (!empty($wp_query->tax_query) AND isset($wp_query->tax_query->queries)) {
                    ...
                    //     if($value['taxonomy'] == 'product_cat' && count($value['terms']) > 1) {
                    //         array_shift($wp_query->tax_query->queries[$key]['terms']);
                    //     }
                    // }
    ...
    public function wp_load_js() {
        ...
        <script>
            ...
            var woof_lang = {
                ...
                'clear_all': "<?php echo apply_filters('woof_clear_all_text', __('Unselect all', 'woocommerce-products-filter')); ?>"
            };
...
//***
        ...
        if (isset($this->settings['optimize_js_files']) AND $this->settings['optimize_js_files']) {
            ...
            wp_enqueue_script('woof_front', WOOF_LINK . 'js/front.js', array('jquery'), '5'); //namdeveloper
                ...
                if (isset($taxonomies[$key])) {
                    if (isset($this->settings['tax'][$key]) && $this->settings['tax'][$key] != 0) {
                        ...
                        if (isset($this->settings["custom_tax_label"][$key]) AND $this->settings["custom_tax_label"][$key]) {
                            $options[$key] = $this->settings["custom_tax_label"][$key];
                        } else {
...
//                );
    ...
    public function woocommerce_before_shop_loop() {
        ...
        if ($woof_set_automatically === 1 AND!isset($_REQUEST['woof_before_shop_loop_done'])) {
            ...
            //Customize
            echo '<div class="filter-and-sort">';
                echo '<div class="wrapper-filter-sort">';
                    echo '<button id="toggleFilter" class="toggle-filter ignore-close-filter">'.__('Filter','glamoutlet').' <strong style="display:none"></strong></button>';
                echo '</div>';
                echo '<div  class="wrapper-filter" style="display:flex">';
...
//***
    ...
    add_action('woocommerce_update_product', function($prod_id, $product) {
        delete_transient('woof_min_max_prices');
    }, 10, 2);
