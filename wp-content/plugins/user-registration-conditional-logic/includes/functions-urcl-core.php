...
function urcl_is_compatible() {
	...
	$ur_plugins_path     = WP_PLUGIN_DIR . URCL_DS . 'user-registration' . URCL_DS . 'user-registration.php';
	$ur_pro_plugins_path = WP_PLUGIN_DIR . URCL_DS . 'user-registration-pro' . URCL_DS . 'user-registration.php';

	if ( ! file_exists( $ur_plugins_path ) && ! file_exists( $ur_pro_plugins_path ) ) {
		return __( 'Please install <code>user-registration-pro</code> plugin to use <code>user-registration-conditional-logic</code> addon.', 'user-registration-conditional-logic' );
	}
	...
	if ( ! is_plugin_active( $ur_plugin_file_path ) && ! is_plugin_active( $ur_pro_plugin_file_path ) ) {
		return __( 'Please activate <code>user-registration-pro</code> plugin to use <code>user-registration-conditional-logic</code> addon.', 'user-registration-conditional-logic' );
	}
	...
	if ( ! is_plugin_active( $ur_pro_plugin_file_path ) ) {
		...
		if ( version_compare( $user_registration_version, '3.0.0', '<' ) ) {
			return __( 'Please update your <code>user-registration-pro</code> plugin(to at least 3.0.0 version) to use <code>user-registration-conditional-logic</code> addon.', 'user-registration-conditional-logic' );
		}
	}
	return 'YES';
				...
				foreach ( $post_content_array as $data ) {
					foreach ( $data as $single_data ) {
						foreach ( $single_data as $field_data ) {
							...
							&& isset( $field_data->general_setting->label ) && isset( $field_data->field_key ) ) {
...
if ( ! function_exists( 'ur_get_conditional_actions' ) ) {
	/**
	 * Get all the conditional actions.
	 *
	 * @param  string $type Conditional logic type.
	 * @return array    $actions.
	 */
	function ur_get_conditional_actions( $type = 'submit' ) {
...
if ( ! function_exists( 'ur_get_conditional_operators' ) ) {
	/**
	 * Get all the conditional operators.
	 *
	 * @param  string $type Conditional logic type.
	 * @return array    $operators.
	 */
	function ur_get_conditional_operators( $type = 'submit' ) {
