...
class URWC_Sync {
	...
	public function edit_account_fields() {
		...
		if ( is_array( $checkout_form_fields ) && isset( $checkout_form_fields[ 'form-' . $edit_account_form_id ] ) ) {
			$checkout_form_fields = $checkout_form_fields[ 'form-' . $edit_account_form_id ];
		} else {
	...
	public function checkout_fields( $checkout ) {
		...
		if ( 0 < $checkout_form_id && $checkout_sync && 0 < count( $checkout_form_fields ) ) {
			...
			echo '<div class="user-registration" data-form-id="' . esc_attr( $checkout_form_id ) . '">';
			foreach ( $profile as $profile_key => $profile_field ) {
				$key = str_replace( 'user_registration_', '', $profile_key );
	...
	public function checkout_process_form( $customer_id, $data ) {
		...
		if ( 0 < $form_id && $checkout_sync ) {
			...
			$mapped_checkout_form_fields = map_checkout_form_fields( $form_fields, $form_id );
			...
			foreach ( $profile as $key => $field ) {
				...
				if ( in_array( $key, $mapped_checkout_form_fields, true ) ) {
