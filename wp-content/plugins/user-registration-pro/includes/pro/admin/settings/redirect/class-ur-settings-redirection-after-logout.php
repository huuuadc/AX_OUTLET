<?php
/**
 * Configure Redirection After Logout
 *
...
if ( ! class_exists( 'User_Registration_Settings_Redirection_After_Logout', false ) ) :

	/**
	 * User_Registration_Settings_Redirection_After_Logout Class.
	 */
	class User_Registration_Settings_Redirection_After_Logout {
		...
		public function __construct() {
			...
			$this->description = esc_html__( 'Redirect users to specific pages after logout based on role', 'user-registration' );
		}
		...
		public function get_settings() {
				...
				array(
					...
					'sections' => array (
						'redirection_after_logout' => array(
							...
							'type'  => 'card',
							'desc'  => '',
							'back_link' => ur_back_link( __( 'Return to Redirection Type', 'user-registration' ), admin_url( 'admin.php?page=user-registration-settings&tab=misc&section=role-based-redirection' ) ),
							'settings' => array(
			...
			add_filter( 'user_registration_admin_field_redirection_after_logout', array( $this, 'redirection_after_logout' ), 10, 2 );

			return apply_filters( 'user_registration_get_settings_' . $this->id, $settings );
		}

		/**
		 * Add Role to Page Mapper for redirection after logout.
		 *
		...
		public function redirection_after_logout( $settings ) {
			...
			$selected_roles_pages = get_option( 'ur_pro_settings_redirection_after_logout', array() );
...
return new User_Registration_Settings_Redirection_After_Logout();
