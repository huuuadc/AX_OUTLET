...
class WC_Brands_Admin {
	...
	public function __construct() {
		...
		add_action( 'brand_add_form_fields', array( $this, 'add_thumbnail_field' ) );
		add_action( 'brand_edit_form_fields', array( $this, 'edit_thumbnail_field' ), 10, 2 );
		add_action( 'created_term', array( $this, 'thumbnail_field_save' ), 10, 3 );
		add_action( 'edit_term', array( $this, 'thumbnail_field_save' ), 10, 3 );
		add_action( 'brand_pre_add_form', array( $this, 'taxonomy_description' ) );
		...
		add_filter(
			...
			function ( $args ) {
				$args['brand'] = [ $this, 'render_brand_filter' ];
				return $args;
			}
		);
	...
	public function add_coupon_brands_fields () {
		...
		// Brands
		?>
		<p class="form-field"><label for="brands"><?php esc_html_e( 'Product brands', 'woocommerce-brands' ); ?></label>
		<select id="brands" name="brands[]" style="width: 50%;"  class="wc-enhanced-select" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Any brand', 'woocommerce-brands' ); ?>">
			<?php
				$category_ids = (array) get_post_meta( $post->ID, 'brands', true );
				$categories   = get_terms( 'brand', 'orderby=name&hide_empty=0' );
		...
		// Exclude Brands
		?>
		<p class="form-field"><label for="exclude_brands"><?php esc_html_e( 'Exclude brands', 'woocommerce-brands' ); ?></label>
		<select id="exclude_brands" name="exclude_brands[]" style="width: 50%;"  class="wc-enhanced-select" multiple="multiple" data-placeholder="<?php esc_attr_e( 'No brands', 'woocommerce-brands' ); ?>">
			<?php
				$category_ids = (array) get_post_meta( $post->ID, 'exclude_brands', true );
				$categories   = get_terms( 'brand', 'orderby=name&hide_empty=0' );
	...
	public function save_coupon_brands ( $post_id ) {
		...
		$exclude_brands = isset( $_POST['exclude_brands'] ) ? array_map( 'intval', $_POST['exclude_brands'] ) : array();

		// Save
		update_post_meta( $post_id, 'brands', $brands );
		update_post_meta( $post_id, 'exclude_brands', $exclude_brands );
	 ...
	 * woocommerce_brand_filter_threshold filter.
	 *
	 * @return void
	 */
	public function render_brand_filter() {
		// phpcs:disable WordPress.Security.NonceVerification
		$brands_count       = (int) wp_count_terms( 'brand' );
		$current_brand_slug = wc_clean( wp_unslash( $_GET['brand'] ?? '' ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		if ( $brands_count <= apply_filters( 'woocommerce_brand_filter_threshold', 100 ) ) {
			wc_product_dropdown_categories(
				[
					...
					'taxonomy'          => 'brand',
					'name'              => 'brand',
					'class'             => 'dropdown_brand',
				]
			);
		} else {
			$current_brand   = $current_brand_slug ? get_term_by( 'slug', $current_brand_slug, 'brand' ) : '';
			$selected_option = '';
			...
			?>
			<select class="wc-brands-search" name="brand" data-placeholder="<?php echo $placeholder; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" data-allow_clear="true">
	...
	public function add_brand_base_setting() {
		...
		add_settings_field(
			'woocommerce_brand_slug',
			__( 'Product brand base', 'woocommerce-brands' ),
			array( $this, 'brand_slug_input' ),
			'permalink',
			'optional'
		);
	 ...
	 */
	public function brand_slug_input() {
		$permalink = get_option( 'woocommerce_brand_permalink', '' );
		?>
		<input name="woocommerce_brand_slug" type="text" class="regular-text code" value="<?php echo esc_attr( $permalink ); ?>" placeholder="<?php echo esc_attr_x( 'brand', 'slug', 'woocommerce-brands' ) ?>" />
		<?php
	}
	...
	public function save_permalink_settings() {
		...
		if ( isset( $_POST['permalink_structure'], $_POST['wc-permalinks-nonce'], $_POST['woocommerce_brand_slug'] ) && wp_verify_nonce( wp_unslash( $_POST['wc-permalinks-nonce'] ), 'wc-permalinks' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			update_option( 'woocommerce_brand_permalink', wc_sanitize_permalink( trim( wc_clean( wp_unslash( $_POST['woocommerce_brand_slug'] ) ) ) ) );
		}
	}
	...
	public function validate_product_base( $value ) {
		if ( '/%brand%/' === trailingslashit( $value['product_base'] ) ) {
	...
	public function process_import( $product, $data ) {
		...
		wp_set_object_terms( $product->get_id(), $brand_ids, 'brand' );
	}
	...
	public function parse_brands_field( $value ) {
		...
		foreach ( $row_terms as $row_term ) {
			...
			foreach ( $_terms as $index => $_term ) {
				$term = term_exists( $_term, 'brand', $parent );

				if ( is_array( $term ) ) {
					$term_id = $term['term_id'];
				} else {
					$term = wp_insert_term( $_term, 'brand', array( 'parent' => intval( $parent ) ) );
	...
	public function get_column_value_brand_ids( $value, $product ) {
		$brand_ids = wp_parse_id_list( wp_get_post_terms( $product->get_id(), 'brand', array( 'fields' => 'ids' ) ) );
		...
		foreach ( $brand_ids as $brand_id ) {
			...
			$ancestor_ids   = array_reverse( get_ancestors( $brand_id, 'brand' ) );

			foreach ( $ancestor_ids as $ancestor_id ) {
				$term = get_term( $ancestor_id, 'brand' );
				if ( $term && ! is_wp_error( $term ) ) {
					$formatted_term[] = $term->name;
				}
			}

			$term = get_term( $brand_id, 'brand' );
