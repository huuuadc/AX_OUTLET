...
class WP_REST_Terms_Product_Controller extends WP_REST_Controller {
 ...
 * @return void
 */
function register_product_custom_field() {
    register_rest_field(
        'product',
        'offline_id',
        array(
            'get_callback'    => function ( $object ) {
                return get_post_meta( $object['id'], 'offline_id', true );
            },
    ...
    register_rest_field(
        ...
        array(
            'get_callback'    => function ( $object ) {
                $brands = get_the_terms($object['id'], 'brand');
                $brands_ids = array();
            ...
            'update_callback' => function ($value, $object, $fieldName){
                ...
                //var_dump($fieldName);
                $new_brand = array();
                wp_delete_object_term_relationships($object->id, 'brand');
                foreach ($value as $k => $v) {
                    $new_brand[] = $v['id'];
                }
                wp_set_object_terms($object->id, $new_brand, 'brand');
            },
            'schema'          => null,
        )
    );
    register_rest_field(
        ...
        array(
            'get_callback'    => function ( $object ) {
                $genders = get_the_terms($object['id'], 'gender');
                $genders_ids = array();
            ...
            'update_callback' => function ($value, $object, $fieldName){
                ...
                //var_dump($fieldName);
                $new_gender = array();
                wp_delete_object_term_relationships($object->id, 'gender');
                foreach ($value as $k => $v) {
                    $new_gender[] = $v['id'];
                }
                wp_set_object_terms($object->id, $new_gender, 'gender');
            },
            'schema'          => null,
        )
    );
    register_rest_field(
        ...
        array(
            'get_callback'    => function ( $object ) {
                return get_term_meta( $object['id'], 'offline_id', true );
            },
            'update_callback' => function ($value, $object, $fieldName){
                return \update_term_meta($object->term_id, $fieldName, $value);
            },
            'schema'          => null,
        )
    );
...
add_action('rest_api_init', 'register_product_custom_field');
