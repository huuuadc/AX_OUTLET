...
class WC_Brands_Coupons {
	...
	public function is_coupon_valid( $valid, $coupon, $discounts = null ) {
		...
		foreach ( $items as $item ) {
			$brands = $this->get_brands( $this->get_product_id( $item->product ) );

			if ( ! empty( array_intersect( $brands, $coupon->included_brands ) ) ) {
				$included_brands_match = true;
			}

			if ( ! empty( array_intersect( $brands, $coupon->excluded_brands ) ) ) {
	...
	public function is_valid_for_product( $valid, $product, $coupon ) {
		...
		$brands = $this->get_brands( $product_id );

		// Check if coupon has a brand requirement and if this product has that brand attached.
		if ( ! empty( $coupon->included_brands ) && empty( array_intersect( $brands, $coupon->included_brands ) ) ) {
			return false;
		}

		// Check if coupon has a brand exclusion and if this product has that brand attached.
		if ( ! empty( $coupon->excluded_brands ) && ! empty( array_intersect( $brands, $coupon->excluded_brands ) ) ) {
			return false;
		}
	 ...
	 * @return array brands
	 */
	private function get_brands( $product_id ) {
		return wp_get_post_terms( $product_id, 'brand', array( 'fields' => 'ids' ) );
	}
	...
	private function set_brand_settings_on_coupon( $coupon ) {
		...
		$included_brands = get_post_meta( $coupon->get_id(), 'brands', true );
		if ( empty( $included_brands ) ) {
			$included_brands = array();
		}

		$excluded_brands = get_post_meta( $coupon->get_id(), 'exclude_brands', true );
