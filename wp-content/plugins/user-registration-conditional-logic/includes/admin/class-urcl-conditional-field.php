...
class URCL_Conditional_field {
	...
	public function __construct() {
		...
		add_filter( 'user_registration_get_learndash_settings', array( $this, 'urcl_admin_conditional_course_setting' ), 10, 1 );
	...
	public function get_users_roles_conditional_output() {
		...
		if ( $user_role_conditions ) {
			...
			$output  = '<div class="urcl-form-settings-container urcl-role-container">';
			$output .= '<div class="urcl-role-conditional-container">';
			$output .= '<div class="urcl-form-settings-logic-wrap urcl-role-logic-wrap" data-group ="condition_1">';
			...
			$output .= '<select class="urcl-form-settings-field urcl-user-role-field" name="user_registration_form_conditional_user_role[condition_1]">';
			...
			$output .= '</div>';
			$output .= '<ul class="urcl-form-settings-logic-box urcl-role-logic-box" data-group ="condition_1" data-last-key="1">';
			$output .= '<li class="urcl-conditional-group" data-key="1">';
			$output .= '<div class="urcl-form-group">';
			$output .= '<select class="urcl-form-settings-field urcl-user-role-field urcl-field-conditional-field-select" name="user_registration_form_fields[condition_1][1]">';
			...
			$output .= '</select></div>';
			$output .= '<div class="urcl-operator"><select class="urcl-form-settings-field urcl-user-role-field" name="user_registration_form_operator[condition_1][1]">';
			...
			$output .= '</select></div>';
			$output .= '<div class="urcl-value"><input class="urcl-form-settings-field urcl-user-role-field" name="user_registration_form_value[condition_1][1]" type="text" /></div>';
	...
	public function get_user_role_conditions_list( $form_id, $user_roles_list, $get_all_fields ) {
		...
		if ( is_array( $condition_role_settings ) && ! empty( $condition_role_settings ) ) {
			$output  = '<div class="urcl-form-settings-container urcl-role-container">';
			$output .= '<div class="urcl-role-conditional-container">';
			...
			foreach ( $condition_role_settings as $conditions ) {
				$output .= '<div class="urcl-form-settings-logic-wrap urcl-role-logic-wrap" data-group ="condition_' . $condition_id . '">';

				$output .= '<div class="urcl-form-settings-assign urcl-assign-role">';
				$output .= '<p class="urcl-assign-label">' . esc_html__( 'Assign', 'user-registration-conditional-logic' ) . '</p>';
				$output .= '<select class="urcl-form-settings-field urcl-user-role-field" name="user_registration_form_conditional_user_role[condition_' . $condition_id . ']">';
				...
				if ( isset( $conditions['conditions'] ) && isset( $conditions['conditions'] ) ) {
					...
					foreach ( $conditions['conditions'] as $condition ) {
						...
						$output .= '<select class="urcl-form-settings-field urcl-user-role-field urcl-field-conditional-field-select" name="user_registration_form_fields[condition_' . $condition_id . '][' . $data_key . ']">';
						$output .= '<option value="">' . esc_html__( '-- Select --', 'user-registration-conditional-logic' ) . '</option>';
						...
						$output .= '</select></div>';
						$output .= '<div class="urcl-operator"><select class="urcl-form-settings-field urcl-user-role-field" name="user_registration_form_operator[condition_' . $condition_id . '][' . $data_key . ']">';
						...
						if ( $selected_urcl_field_type == 'checkbox' || $selected_urcl_field_type == 'radio' || $selected_urcl_field_type == 'select' || $selected_urcl_field_type == 'country' || $selected_urcl_field_type == 'billing_country' || $selected_urcl_field_type == 'shipping_country' || $selected_urcl_field_type == 'select2' || $selected_urcl_field_type == 'multi_select2' ) {
							...
							$output .= '<select name="user_registration_form_value[condition_' . $condition_id . '][' . $data_key . ']" class="urcl-form-settings-field urcl-user-role-field">';
							...
							$output .= '</select>';
								...
								$output .= '<input name="user_registration_form_value[condition_' . $condition_id . '][' . $data_key . ']" value="' . esc_attr( $fields_data[2] ) . '" class="urcl-form-settings-field urcl-user-role-field" type="text" />';
						}
						$output .= '</div>';
						$output .= '<span class="add">';
						$output .= esc_html__( 'AND', 'user-registration-conditional-logic' );
	...
	public function conditional_field( $id, $admin_data ) {
		...
		if ( '' != $conditional_settings ) {
			...
			echo "<div class='user-registration-field-option-group ur-advance-setting-block closed'>";

			echo '<h2 class="ur-toggle-heading closed">' . __( 'Conditional Settings', 'user-registration-conditional-logic' ) . '</h2>';
	...
	public function urcl_admin_conditional_role_setting( $setting ) {
		...
		$form_setting = array(
			array(
				...
				'default'           => ur_get_single_post_meta( $form_id, 'user_registration_form_setting_enable_assign_user_role_conditionally', false ),
	...
	/**
	 * Conditionally assign course.
	 *
	 * @param  array $setting
	 * @return array $setting
	 */
	public function urcl_admin_conditional_course_setting( $setting ) {

		$form_id        = $setting['form_id'];
		$course_setting = array(
			array(
				'type'              => 'toggle',
				'label'             => __( 'Enable Assign Learndash course Conditionally', 'user-registration-learndash' ),
				'description'       => '',
				'required'          => false,
				'id'                => 'user_registration_form_setting_enable_assign_learndash_course_conditionally',
				'class'             => array( 'ur-enhanced-select', 'urcl-form-settings-enable-assign-learndash-course' ),
				'input_class'       => array(),
				'custom_attributes' => array(),
				'default'           => ur_get_single_post_meta( $form_id, 'user_registration_form_setting_enable_assign_learndash_course_conditionally', false ),
				'tip'               => __( 'Assign certain course only if your conditions are met.', 'user-registration-learndash' ),
			),
		);

		$setting['setting_data'] = array_merge( $setting['setting_data'], $course_setting );
		return $setting;
	}
}
